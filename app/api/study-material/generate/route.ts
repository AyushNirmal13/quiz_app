        ...
        try {
          ...
          for (const q of questions) {
            if (
              ...
              const keyStr = q.question.toLowerCase().replace(/\s+/g, " ").slice(0, 80);
              if (!seen.has(keyStr)) {
                seen.add(keyStr);
                deduplicated.push({
          ...
          // If model not found (404), try the next model name with this same key
          if (msg.includes("404") || msg.includes("not found") || msg.includes("not_found")) {
            console.warn(`Model "${modelName}" not found for this key, trying next model...`);
            continue;
          }
